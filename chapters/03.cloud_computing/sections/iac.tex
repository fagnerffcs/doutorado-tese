...
\begin{itemize}
    ...
    \item \textbf{Application Layer (Helm):} To define the reference architecture itself. The PANOPTES architecture is packaged as Helm Charts, allowing the complex orchestration of Prometheus, Jaeger, and OpenTelemetry components to be instantiated deterministically. This ensures that the reference architecture is an artifact that can be audited, versioned, and shared with the community, preventing the "it works on my machine" bias.
\end{itemize}