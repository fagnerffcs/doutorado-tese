...
\begin{itemize}
    \item \textbf{Virtualization Layer (Multipass):} Employed to simulate the Infrastructure as a Service (IaaS) baseline. Multipass is utilized to programmatically launch and orchestrate native virtual machines with identical hardware allocations (CPU, memory, and network interfaces) backed by the native Windows hypervisor (Hyper-V or WSL2), bypassing the VirtualBox I/O bottleneck.
