...
In the storage layer, for long-term retention, observability backends (such as Prometheus for metrics or Elasticsearch/Loki for logs) are deployed as StatefulSets. Unlike stateless applications, these components require stable network identities and persistent storage. They utilize Persistent Volume Claims (PVCs) mapped to a StorageClass to request physical storage from the underlying infrastructure (IaaS), ensuring that telemetry data survives Pod restarts or node failures \cite{luksa_2018, hausenblas_2023}.