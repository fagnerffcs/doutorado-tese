...
\begin{enumerate}
    ...
    \item \textbf{Blind Fault Injection ($T_0$):} Faults are injected programmatically via Chaos Mesh by applying a calibrated perturbation from our multi-profile fault pool. To ensure experimental neutrality and eliminate selection bias, a blind randomized injector selects both the **target microservice** and the **type of chaos** at runtime:
    \begin{itemize}
        \item \textbf{Fault Type Pool:} Selects randomly between \texttt{NetworkChaos} (inducing network delay), \texttt{PodChaos} (triggering a forced pod kill), or \texttt{StressChaos} (saturating CPU resources).
        \item \textbf{Target Microservice Pool:} Selects randomly from the critical transaction path components ($\text{Pool} = \{\texttt{payment}, \texttt{cart}, \texttt{shipping}, \texttt{currency}\}$).
    \end{itemize}
    The exact RFC3339 timestamp of the Kubernetes API Server submission is recorded as the authoritative injection start time ($T_0$).
...
\input{chapters/06.experiment/subsections/experiment-execution/automated-orchestration}

\input{chapters/06.experiment/subsections/experiment-execution/testbed-comissioning}